...
console.log('feeIds', feeIds);
  ...
  private calculateReferralStatus(enquiry: any, parentNumber: string): string {
    ...
    const referralPhone = normalizePhone(referral.phone);

    // Check if referrer phone matches parent phone
    const referrerMatches = referrerPhone && parentPhone && referrerPhone === parentPhone;
    
    // Check if referral phone matches parent phone
    const referralMatches = referralPhone && parentPhone && referralPhone === parentPhone;

    // Verification logic with phone number matching
    const referrerVerified = referrer.verified === true && referrerMatches;
    const referralVerified = referral.verified === true && referralMatches;
    ...
    const pipeline: any[] = [
      ...
      // 2️⃣ ADD COMPUTED FIELDS FOR STAGE TRACKING
      {
        $addFields: {
              ...
              $map: {
                ...
                in: {
                  $and: [
                    {
                      $or: [
                        { $eq: ["$$stage.stage_name", "Registration"] },
                      ]
                    },
                    {
                      $or: [
          ...
          is_excluded_from_ql: {
            $or: [
              { $eq: ["$status", "Closed"] },
              // Add more exclusion conditions based on your tags/status field
              // For example, if you have a tags array:
              // { $in: ["Spam", "$tags"] },
              // { $in: ["Duplicate", "$tags"] },
            ]
          }
      ...
      // 3️⃣ GROUP BY DIMENSIONS (Based on group_by array)
      {
        $group: {
          ...
          total_raw_leads: {
            $sum: { $cond: ["$has_enquiry_stage", 1, 0] }
          },

          // QL = Enquiries with "School visit" stage AND not excluded
          ...
          total_appointment: {
            $sum: { $cond: ["$has_school_visit_stage", 1, 0] }
          },
          ...
          // Registration
          total_registration: {
            $sum: { $cond: ["$has_registration_stage", 1, 0] }
          },
