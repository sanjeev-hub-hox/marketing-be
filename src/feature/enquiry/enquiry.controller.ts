...
export class EnquiryController {
  ...
  async getReportForSourceConversion(
    ...
    const cacheKey = `source-wise-conversion-report`;
    const cachedData = await this.redisInstance?.getData(cacheKey);
    ...
    const finalRows = await this.enquiryService.sourceConversionReport();
    const reportFile = await this.enquiryService.generateAndUploadSourceConversionCsv(finalRows);
